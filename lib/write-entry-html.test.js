const argumentsHelper = require('./common/arguments-helper');
...
jest.mock('./common/arguments-helper')
jest.mock('path')
jest.mock('fs')
...
test('getting relevant paths', () => {
    ...
    when(argumentsHelper.getArgumentOrEnvValue).calledWith('source-root', 'SOURCE_ROOT').mockReturnValue(sourceRoot)
    when(argumentsHelper.getArgumentOrEnvValue).calledWith('file-path', 'FILE_PATH').mockReturnValue(filePath)
    when(argumentsHelper.getArgumentOrEnvValue).calledWith('target-root', 'TARGET_ROOT').mockReturnValue(targetRoot)
